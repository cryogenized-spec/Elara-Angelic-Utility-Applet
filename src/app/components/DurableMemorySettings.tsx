...
export function DurableMemorySettings() {
  ...
  async function handleDelete(id: string) { try { await deleteMemory(id); setError(null); await refresh(); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Could not delete that memory.'); } }
  ...
  return <div className="memory-settings">
    <div className="memory-settings__header"><div><strong>Durable memory</strong><span>Explicit notes Elara may retrieve during a Gemini interaction. Conversation history remains separate.</span></div><button type="button" onClick={() => { resetDraft(); setCreating(true); }}>New memory</button></div>
    <div className="memory-settings__filters"><label><span>View</span><select value={scopeFilter} onChange={(event) => setScopeFilter(event.target.value)}><option value="all">All memories</option><option value="active">Active</option><option value="global">Global only</option><option value="archived">Archived</option>{folderState.folders.map((folder) => <option key={folder.id} value={`folder:${folder.id}`}>{folderPath(folder.id, folderState.folders)}</option>)}</select></label><small>{memories.length} stored · {filtered.length} shown · retrieval remains bounded</small></div>
    {error && <div className="memory-settings__error" role="alert">{error}</div>}
    {(creating || editingId) && <div className="memory-editor"><div className="memory-editor__title">{editingId ? 'Edit memory' : 'Create memory'}</div><label><span>Title</span><input value={draft.title} maxLength={160} onChange={(event) => setDraft((current) => ({ ...current, title: event.target.value }))} /></label><label><span>Memory body</span><textarea value={draft.body} maxLength={50000} onChange={(event) => setDraft((current) => ({ ...current, body: event.target.value }))} /></label><div className="memory-editor__grid"><label><span>Kind</span><select value={draft.kind} onChange={(event) => setDraft((current) => ({ ...current, kind: event.target.value as MemoryKind }))}>{MEMORY_KINDS.map((kind) => <option key={kind} value={kind}>{kind.replace('_', ' ')}</option>)}</select></label><label><span>Scope</span><select value={draft.folderId} onChange={(event) => setDraft((current) => ({ ...current, folderId: event.target.value }))}><option value="">Global</option>{folderState.folders.map((folder) => <option key={folder.id} value={folder.id}>{folderPath(folder.id, folderState.folders)}</option>)}</select></label><label><span>Importance</span><input type="number" min="0" max="1" step="0.05" value={draft.importance} onChange={(event) => setDraft((current) => ({ ...current, importance: event.target.value }))} /></label><label><span>Confidence</span><input type="number" min="0" max="1" step="0.05" value={draft.confidence} onChange={(event) => setDraft((current) => ({ ...current, confidence: event.target.value }))} /></label></div><label><span>Tags</span><input value={draft.tags} maxLength={2048} placeholder="identity, preference, project" onChange={(event) => setDraft((current) => ({ ...current, tags: event.target.value }))} /></label><div className="memory-editor__actions"><button type="button" onClick={resetDraft}>Cancel</button><button type="button" className="primary" onClick={() => void saveDraft()}>{editingId ? 'Save changes' : 'Create memory'}</button></div></div>}
    {loading ? <p className="memory-settings__empty">Loading durable memories…</p> : filtered.length === 0 ? <div className="memory-settings__empty"><strong>No memories here.</strong><span>Create an explicit durable note, or change the filter.</span></div> : <div className="memory-list">{filtered.map((memory) => <article className="memory-card" key={memory.id}><div className="memory-card__meta"><span>{memory.kind.replace('_', ' ')}</span><span>{memory.lifecycle}</span><span>{memory.folderId ? folderPath(memory.folderId, folderState.folders) : 'Global'}</span></div><h3>{memory.title}</h3><p>{memory.body}</p>{memory.tags.length > 0 && <div className="memory-card__tags">{memory.tags.map((tag) => <span key={tag}>#{tag}</span>)}</div>}<small>Confidence {Math.round(memory.confidence * 100)}% · Importance {Math.round(memory.importance * 100)}% · Recalled {memory.recallCount}× · Reinforced {memory.reinforcementCount}×</small><small>Observed {formatDate(memory.observedAt)} · Updated {formatDate(memory.updatedAt)} · {sourceLabel(memory.source.source)}</small><div className="memory-card__actions"><button type="button" onClick={() => beginEdit(memory)}>Edit</button>{memory.lifecycle === 'archived' ? <button type="button" onClick={() => void handleReinforce(memory.id)}>Restore</button> : <button type="button" onClick={() => void handleArchive(memory.id)}>Archive</button>}<button type="button" onClick={() => void handlePromote(memory.id)}>Promote</button><button type="button" className="danger" onClick={() => void handleDelete(memory.id)}>Delete</button></div></article>)}</div>}
  </div>;
}
