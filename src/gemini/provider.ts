...
export const geminiTurnPort: GeminiTurnPort = {
  async *streamReply(request: GeminiTurnRequest, signal?: AbortSignal): AsyncGenerator<GeminiStreamEvent> {
    const startedAt = performance.now(); const requestId = crypto.randomUUID(); let interactionId: string | undefined;
    if (signal?.aborted) { yield { type: 'cancelled' }; return; }
    try {
      const response = await fetch(WORKER_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: request.model || DEFAULT_GEMINI_MODEL, input: request.input, previousInteractionId: request.previousInteractionId, generationConfig: request.generationConfig, systemInstruction: request.systemInstruction }), signal });
      ...
      try {
        while (true) {
          ...
          while (boundary >= 0) {
            ...
            const event = parsed.data; const eventType = parsed.eventType; const eventInteractionId = interactionIdFrom(event); if (eventInteractionId) interactionId = eventInteractionId;
            if (eventType === 'interaction.created') { const interaction = asRecord(event.interaction); const id = readString(interaction, 'id') ?? interactionId ?? 'unknown'; interactionId = id; yield { type: 'interaction-created', interactionId: id, model: readString(interaction, 'model') ?? (request.model || DEFAULT_GEMINI_MODEL) }; continue; }
            if (eventType === 'interaction.in_progress' || eventType === 'interaction.status_update' || eventType === 'interaction.status' || eventType === 'interaction.updated' || eventType === 'interaction.requires_action') { const status = readString(event, 'status') ?? readString(asRecord(event.interaction), 'status') ?? eventType.replace('interaction.', ''); if (interactionId) yield { type: 'interaction-status', interactionId, status }; continue; }
            ...
            if (eventType === 'interaction.completed') { const interaction = asRecord(event.interaction); interactionId = readString(interaction, 'id') ?? interactionId; const status = readString(interaction, 'status') ?? 'completed'; yield { type: 'completed', interactionId: interactionId ?? 'unknown', status, durationMs: Math.max(1, Math.round(performance.now() - startedAt)), usage: readUsage(interaction.usage) ?? readUsage(event.usage) }; return; }
