...
const css2UrlSchema = z.string().url().refine((value) => { try { const url = new URL(value); return url.protocol === 'https:' && url.hostname === 'fonts.googleapis.com' && url.pathname === '/css2'; } catch { return false; } }, 'Use a Google Fonts CSS2 URL from fonts.googleapis.com.');
...
function loadCustomGoogleFont(stylesheetUrl: string) { const existing = document.querySelector<HTMLLinkElement>(`link[data-elara-custom-font="${stylesheetUrl}"]`); if (existing) return; const link = document.createElement('link'); link.rel = 'stylesheet'; link.href = stylesheetUrl; link.dataset.elaraCustomFont = stylesheetUrl; document.head.appendChild(link); }

export function SettingsScreen({ font, onFontChange, fontSize, onFontSizeChange, portraitScale, onPortraitScaleChange, portraitBackground, onPortraitBackgroundChange, selectedModel, geminiSettings, onModelChange, onGeminiSettingsChange, onResetGeminiSettings, character, onCharacterChange, chatAppearance, onChatAppearanceChange, roleplay, onRoleplayChange, onBack }: {
  ...
  character: CharacterProfile; onCharacterChange: (profile: CharacterProfile) => void;
  chatAppearance: ChatAppearancePreferences; onChatAppearanceChange: (value: ChatAppearancePreferences) => void;
  roleplay: RoleplayPreferences; onRoleplayChange: (value: RoleplayPreferences) => void;
  onBack: () => void;
}) {
  ...
  return <main className="settings-screen" style={{ fontFamily: fontFamilyForCss(font), '--body-font-size': `${fontSize}px` } as React.CSSProperties}>
    ...
    <div className="settings-layout">
      ...
      <section className="settings-panel" aria-live="polite">
        {section === 'appearance' && <div className="settings-copy"><span className="panel-kicker">PRESENCE</span><h2>Appearance</h2><p>Character presentation and the independent Android chat backdrop live here.</p><RangeSlider id="portrait-scale" label="Character presentation scale" min={1} max={3} value={portraitScale} valueLabel={`${portraitScale}×`} minLabel="1×" maxLabel="3×" onChange={(value) => onPortraitScaleChange(Math.min(3, Math.max(1, value)) as PortraitScale)} /><div className="background-picker"><div className="background-picker__header"><strong>Character banner ambience</strong><span>Presentation layer only.</span></div><div className="background-options" role="radiogroup" aria-label="Character banner background">{portraitBackgrounds.map((option) => { const active = portraitBackground === option.id; return <button key={option.id} type="button" role="radio" aria-checked={active} className={`background-option background-option--${option.id}${active ? ' is-active' : ''}`} onClick={() => onPortraitBackgroundChange(option.id)}><span>{option.label}</span><small>{option.description}</small></button>; })}</div></div><ChatAppearanceSettings value={chatAppearance} onChange={onChatAppearanceChange} /></div>}
        ...
        {section === 'security' && <div className="settings-copy"><span className="panel-kicker">SECURITY</span><h2>API Lockbox</h2><p>The Lockbox is the app's protected configuration boundary. Secrets stay outside the browser UI.</p><WorkerHealthPanel /></div>}
      </section>
    </div>
  </main>;
}
