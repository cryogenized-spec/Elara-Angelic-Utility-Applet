...
Drive remains Google's file authority; Elara keeps no second Drive catalogue. Pass 1 exposes `drive.searchFiles` (app-file capability), `drive.searchLibrary` (separate, sensitive library-read consent) and `drive.getFile` as reads, plus `drive.downloadFile`, which copies one file into a local artifact instead of returning bytes to the model.
