...
const settingsSections = [
  ...
  { id: 'memory', label: 'Memory', icon: 'markdown' as const },
  ...
  { id: 'clickup', label: 'ClickUp', icon: 'shield' as const },
...
export function SettingsScreen({
  ...
  chatTextSize: number;
  onChatTextSizeChange: (fontSize: number) => void;
    ...
    <main className="settings-screen" style={{ fontFamily: fontFamilyForCss(font), '--body-font-size': '15px' } as React.CSSProperties}>
      ...
      <div className="settings-layout">
        ...
        <section className="settings-panel" aria-live="polite">
          ...
          {section === 'memory' && <div className="settings-copy"><span className="panel-kicker">CONTINUITY</span><h2>Memory & continuity</h2><p>Choose how readily Elara remembers, how naturally she brings memories into conversation, and which parts of everyday life may be learned automatically. Organized memory topics follow; advanced Memory Bank controls remain below.</p><MemoryContinuitySettings /><SemanticMemoryFiles /><DurableMemorySettings /></div>}
          ...
          {section === 'google' && <div className="settings-copy"><span className="panel-kicker">ACCOUNT & AUTHORIZATION</span><h2>Google</h2><p>Connect Google Workspace once, review the requested permissions in Google’s consent screen, and let Elara reflect the scopes you actually grant. Authorization state and secrets remain owned by the protected OAuth authority.</p><GoogleOAuthSettings /><WorkspaceShortcutSettings /></div>}
          {section === 'clickup' && <div className="settings-copy"><span className="panel-kicker">ACCOUNT & AUTHORIZATION</span><h2>ClickUp</h2><p>Connect ClickUp to Elara’s first-party MCP through your paired Worker. ClickUp controls which Workspaces the OAuth token can reach; Elara separately controls which model actions may execute.</p><ClickUpOAuthSettings /></div>}
