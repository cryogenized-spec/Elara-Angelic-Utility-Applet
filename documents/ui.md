...
Canonical shell width is capped at `520px`; the 9:16 Android portrait composition is the reference geometry. Touch controls use a `44px` minimum shell control token. The chat header control cluster is a durable two-row contract: hamburger and Kanban share the first row with the shared control gap, while the full-width Workspace launcher sits directly beneath them. Character presentation is frameless in portrait mode: the former gradient/banner backplate and visible identity/presence card are absent, while the 4:5 portrait remains pinned top-right as foreground chrome. In portrait mode neither the portrait nor the top control cluster reserves conversation height: the conversation scroller extends to the shell top behind them, with controls and artwork layered above chat content. Landscape artwork retains its in-flow presentation contract. The character name remains present as a visually hidden level-one heading so screen-reader heading navigation survives the frameless treatment. The composer is a four-track row: attachment, flexible editor, VTT and send. The editor grows to roughly ten visible lines, then scrolls internally; conversation space shrinks instead of being pushed outside the shell.
...
Appearance hex-colour fields are plain-text boundaries. Whitespace (including internal/non-breaking whitespace), zero-width/bidi formatting marks and common pasted rich-text decoration around a complete six-digit colour are removed before validation; a complete valid colour commits immediately, while incomplete manual edits remain transactional until blur/Enter. Appearance state is optimistic: the latest local choice renders immediately while IndexedDB writes remain serialized, and stale acknowledgements from older queued writes must never repaint older appearance state over a newer edit.
