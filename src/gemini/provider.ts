...
async function readErrorResponse(response: Response): Promise<Error & { status?: number }> {
...
function parseSseBlock(block: string): { eventType: string; data: Record<string, unknown> } | null {
  let eventType = '';
  let dataText = '';
  ...
  try {
    return { eventType, data: asRecord(JSON.parse(dataText)) };
  } catch {
    return null;
  }
}
  ...
  async *streamReply(request: GeminiTurnRequest, signal?: AbortSignal): AsyncGenerator<GeminiStreamEvent> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const normalized = normalizeGeminiError(error, { requestId, durationMs: Math.max(1, Math.round(performance.now() - startedAt)) });
        yield { type: 'failed', error: normalized };
        return;
      }

      if (!response.body) {
        const normalized = normalizeGeminiError(new Error('Gemini Worker returned no streaming body.'), { requestId });
        yield { type: 'failed', error: normalized };
        return;
      }
        ...
        while (true) {
          ...
          while (boundary >= 0) {
            ...
            if (eventType === 'interaction.in_progress' || eventType === 'interaction.status_update' || eventType === 'interaction.status' || eventType === 'interaction.updated' || eventType === 'interaction.requires_action') {
              const status = readString(event, 'status') ?? readString(asRecord(event.interaction), 'status') ?? eventType.replace('interaction.', '');
            ...
            if (eventType === 'step.start') {
              ...
              for (const summary of summaryParts) {
                ...
                if (text) yield { type: 'thought-summary-delta', index, text };
              }
            ...
            if (eventType === 'error') {
              ...
              const message = readString(providerError, 'message') ?? 'Gemini Worker returned a streaming error.';
      ...
      const normalized = normalizeGeminiError(new Error('Gemini Worker stream ended without an explicit interaction.completed event.'), { requestId, interactionId, durationMs: Math.max(1, Math.round(performance.now() - startedAt)) });
      yield { type: 'failed', error: normalized };
    } catch (cause) {
