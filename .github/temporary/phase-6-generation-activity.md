# TEMPORARY — Phase 6 Generation Activity Ledger

> **Temporary working record. Not canonical documentation.**
> This file exists only while Phase 6 is being implemented, verified and human-tested. Once the phase is accepted, durable facts are to be distilled into the canonical documentation register and this temporary file removed.
>
> Status vocabulary is intentionally binary: **ACCOMPLISHED** means the implementation step exists on this branch and satisfies its intended code-level contract to the confidence stated; **NOT YET ACCOMPLISHED** means a required part, proof, or acceptance gate remains outstanding. A green CI result is evidence, not a substitute for the functional criterion.

## Chronological Phase 6 record

### 6.1 Reuse the existing generation path
...
### 6.2 Replace the split live/persisted trace model with one structured activity model
...
### 6.3 Preserve provider reasoning summaries without exposing hidden reasoning payloads
...
### 6.4 Model tool activity from canonical tool events and names
...
### 6.5 Represent durable-memory retrieval as application activity, not a fake Gemini tool
...
### 6.6 Render one Generation Activity component for live and completed turns
...
### 6.7 Bound activity growth and anchor the beginning of a generated turn
...
### 6.9 Persist a user-selectable Generation Activity accent colour
...
### 6.10 Remove superseded duplicate operations and presentation paths
...
### 6.11 Standardise project command invocation

**Status: NOT YET ACCOMPLISHED**  
**Confidence in intended change: 99%**

The repository already exposes `npm run e2e` and pins `@playwright/test`. CI still needs to be checked and, where it invokes Playwright through `npx`, changed to the existing npm script interface (`npm run e2e -- ...`). This keeps `package.json` as the project command authority and removes an unnecessary second invocation style.

### 6.12 Unit and integration edge-case coverage
...
The complete unit suite has not yet been observed passing on the final Phase 6 head, so this gate remains open.

### 6.13 Adversarial browser pass — deliberately try to break the feature
...
### 6.14 Full repository verification on the final implementation head
...
### 6.16 Canonical documentation update
