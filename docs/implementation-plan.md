...
- Version the Lockbox metadata to include selected security mode and required authentication metadata.
...
- Use a dedicated mobile unlock surface with type=password, inputmode=numeric, numeric pattern, and an autocomplete value appropriate for a persistent PIN rather than an OTP.
