...
const toolProperties: Record<string, Record<string, unknown>> = {
  ...
  'tasks.listTaskLists': { pageToken: stringProperty('Optional pagination token.'), maxResults: { type: 'integer', minimum: 1, maximum: 1000, description: 'Maximum task lists to return on this page.' } },
  ...
  'tasks.moveTask': {
    taskListId: stringProperty('Task-list id.'),
    taskId: stringProperty('Task id.'),
    parent: stringProperty('Optional destination parent task id. Omit to make the task top-level.'),
    previous: stringProperty('Optional previous sibling id. Omit to place the task first among its destination siblings.'),
  },
