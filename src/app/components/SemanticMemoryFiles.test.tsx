...
describe('SemanticMemoryFiles (memory topics surface)', () => {
  ...
  it('creates the first topic from canonical evidence through the production UI pipeline', async () => {
    const source = await saveMemory({ title: 'Project owner', body: 'Zuhayr is the owner of the project.' });
    const before = await db.memories.toArray();
    act(() => { root.render(<SemanticMemoryFiles />); });
    await waitForText('No summaries yet.');
    ...
    act(() => buttonByText('Create summary').click());
    await waitForText('Topic ready.');
    const files = await listSemanticFiles();
    expect(files).toHaveLength(1);
    expect(files[0]).toMatchObject({ title: 'Zuhayr', sourceMemoryIds: [source.id] });
    expect(await db.memories.toArray()).toEqual(before);
    expect(container.textContent).toContain('Underlying memories');
  });

  it('explains missing evidence without manufacturing a summary', async () => {
    act(() => { root.render(<SemanticMemoryFiles />); });
    await waitForText('No summaries yet.');
    act(() => buttonByText('Create topic from memories').click());
    const input = container.querySelector('form input') as HTMLInputElement;
    ...
    act(() => buttonByText('Create summary').click());
    await waitForText('No eligible memories match');
  ...
  it('groups summary files by concept and exposes provenance at a glance', async () => {
    ...
    await writeSemanticFile(fileTemplate({ id: 'semantic_ui_project', kind: 'project', title: 'Elara app', aliases: [], summary: 'The companion app.', sourceMemoryIds: [source.id], updatedAt: source.updatedAt }), 0);
