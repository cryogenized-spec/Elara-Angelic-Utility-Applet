...
- ClickUp OAuth writes use the same installation signing authority; ClickUp OAuth state is durable, redirect-bound, expiring and single-use. Connection writes encode the browser gesture generation inside the HMAC-covered nonce while the normal timestamp remains a freshness check. The ClickUp vault persists the highest admitted intent generation, rejects older/equal writes before mutation, and exposes that non-secret high-water mark with connection-state so browser tabs cannot mistake a pre-egress local marker for a settled Worker write. Stale clients without the intent nonce fail closed and must refresh.
