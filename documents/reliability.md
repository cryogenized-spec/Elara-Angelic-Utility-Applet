...
These tests certify application, credential, authority, test, CI and deployment boundaries. Indirect prompt injection has an application-enforced containment boundary rather than relying on model obedience alone. Successful Calendar/Tasks/Gmail/Drive/Docs/Sheets/YouTube reads intrinsically taint the model turn, while explicit `trust: untrusted-external` remains a second tripwire. Workspace provider JSON is bounded before parsing and semantic projections apply tighter field/list/text budgets with visible truncation metadata. A fixed provenance instruction is frozen into the complete Gemini tool interaction, but application-owned declared-tool admission, capability checks, schemas and confirmation remain the enforcement authorities. A mutation proposed by a later model continuation after tainted external content is marked `untrustedContext`; the confirmation broker displays an external-content warning, leaves the action unselected, and disables approval until the human explicitly selects it. Mutations emitted in the same model batch as a read are not retroactively tainted because the model had not yet received that read result. Grouped items likewise start unselected and there is no approve-all control. Adversarial tests require hostile Workspace content asking for unrelated mutations to remain unable to manufacture tool authority. Broader cross-turn provenance/taint propagation and semantic information-flow analysis remain future security work rather than a solved claim.
