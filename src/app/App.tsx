...
import { subscribeKanbanFocus, type KanbanFocusTarget } from '../kanban/focus';
...
export function App() {
  ...
  const [kanbanFocusTarget, setKanbanFocusTarget] = useState<KanbanFocusTarget | null>(null);
  useEffect(() => startBoardSync(), []);
  const [settingsOpen, setSettingsOpen] = useState(false);
  const [settingsSection, setSettingsSection] = useState<SettingsSection>('appearance');
  useEffect(() => subscribeKanbanFocus((target) => {
    setSidebarOpen(false);
    setSettingsOpen(false);
    setKanbanFocusTarget(target);
    setKanbanOpen(true);
  }), []);
  ...
  if (kanbanOpen) return <main style={{ ...appStyle, fontFamily: fontFamilyForCss(uiSettings.font) } as React.CSSProperties}><KanbanScreen focusTarget={kanbanFocusTarget} onBack={() => { setKanbanOpen(false); setKanbanFocusTarget(null); }} onSettings={() => { setSettingsSection('google'); setSettingsOpen(true); }} /></main>;
  return <main className="app-shell" style={{ ...appStyle, fontFamily: fontFamilyForCss(uiSettings.font) } as React.CSSProperties}>
    ...
    {/* Compact two-row control cluster: hamburger + Kanban on the first row,
        Workspace directly beneath them (layout.css). */}
    <div className="control-stack" aria-label="Application controls">
      <div className="control-stack__top">
        ...
        <button type="button" className="kanban-launcher" onClick={() => { setKanbanFocusTarget(null); setKanbanOpen(true); }}>Kanban</button>
      </div>
