...
function formatSourceMemory(memory: DurableMemory): string {
  const body = memory.body.length > SEMANTIC_SOURCE_CHARACTERS - memory.title.length - 2
    ? `${memory.body.slice(0, SEMANTIC_SOURCE_CHARACTERS - memory.title.length - 2).trimEnd()}…`
    : memory.body;
  return `Source "${memory.title}": ${body}`;
}
