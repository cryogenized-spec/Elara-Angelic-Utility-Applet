...
if (!clickUpOauthProviderSource.includes("CLICKUP_PERSONAL_CREDENTIAL_PREFIX = 'elara-clickup-personal-v1:'") || !clickUpOauthProviderSource.includes('personalClickUpCredential') || !clickUpOauthProviderSource.includes('credential.startsWith(CLICKUP_PERSONAL_CREDENTIAL_PREFIX)') || !clickUpOauthProviderSource.includes('return `Bearer ${credential}`')) throw new Error('Reliability gate: ClickUp provider auth must preserve explicit personal-token and OAuth Authorization forms.');
if (!clickUpOauthVaultSource.includes("name: 'AES-GCM'") || !clickUpOauthVaultSource.includes('CLICKUP_OAUTH_VAULT_KEY')) throw new Error('Reliability gate: ClickUp provider credentials must remain AES-GCM encrypted with the dedicated vault key.');
...
if (!clickUpOauthRoutesSource.includes('verifySignedWrite') || !clickUpOauthRoutesSource.includes("'/clickup/oauth/personal-token'") || !workerCompositionSource.includes('handleClickUpOAuthRoute')) throw new Error('Reliability gate: public ClickUp credential writes must retain signed admission through the reviewed route.');
