  ...
  useEffect(() => {
    setSelectedVariants((current) => {
      ...
      for (const entry of grouped) {
        ...
        if (!(key in next) || previousCount !== count) next[key] = count - 1;
      ...
      {grouped.map(({ message, variants }) => {
        ...
        return <article className="message message-assistant assistant-glow" key={groupId}>
          ...
          <div className="message-actions">
            <button type="button" className="message-action" aria-label="Regenerate response" onClick={() => onRegenerate(selected.id)}>Regenerate</button>
          </div>
        </article>;
      })}
    </div>
