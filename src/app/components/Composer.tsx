...
export function Composer({ draft, status, onDraftChange, onSend, onCancel }: { draft: string; status: ProviderStatus; onDraftChange: (value: string) => void; onSend: () => void; onCancel: () => void; }) {
  ...
  useEffect(() => { const textarea = textareaRef.current; if (!textarea) return; textarea.style.height = 'auto'; const nextHeight = Math.min(MAX_HEIGHT, Math.max(42, textarea.scrollHeight)); textarea.style.height = `${nextHeight}px`; textarea.style.overflowY = textarea.scrollHeight > MAX_HEIGHT ? 'auto' : 'hidden'; }, [draft]);
  function handleKeyDown(event: KeyboardEvent<HTMLTextAreaElement>) { if (event.key !== 'Enter' || event.shiftKey || event.nativeEvent.isComposing) return; if (status === 'streaming' || !draft.trim()) return; event.preventDefault(); onSend(); }
  return <>
