...
describe('media hand-off', () => {
  ...
  describe('on Android', () => {
    it('wraps the destination in an intent URI with the required terminator', () => {
    ...
    it('does not pin a package, because pinning would suppress the app picker', () => {
      ...
      const href = mediaHandoffHref(item({ intent: 'listen' }), ANDROID);
  ...
  it('falls back to the web URL for a link it cannot parse, instead of emitting a malformed intent', () => {
    ...
    expect(mediaHandoffHref(item(), ANDROID)).toContain('intent://');
  });
  ...
  it('refuses to build an intent around a non-https link', () => {
    ...
    expect(mediaHandoffHref(insecure, ANDROID)).not.toContain('intent://');
    expect(mediaHandoffHref(item(), ANDROID)).toContain('scheme=https;');
  });
  ...
  describe('platform detection', () => {
    type NavigatorLike = { userAgent?: string; userAgentData?: { platform?: string } };
    ...
    it('falls back to the UA when the brand is missing or blank', () => {
      const restoreBlank = withNavigator({ userAgent: 'Mozilla/5.0 (Linux; Android 14; Pixel 8)', userAgentData: { platform: '   ' } });
      try {
        expect(detectHandoffPlatform()).toEqual({ isAndroid: true });
      } finally {
        restoreBlank();
      ...
      const restoreMissing = withNavigator({ userAgent: 'Mozilla/5.0 (Linux; Android 14; Pixel 8)' });
      try {
        expect(detectHandoffPlatform()).toEqual({ isAndroid: true });
      } finally {
        restoreMissing();
      ...
      const restoreDesktop = withNavigator({ userAgent: 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)' });
      try {
        expect(detectHandoffPlatform()).toEqual({ isAndroid: false });
      } finally {
        restoreDesktop();
