...
export function Sidebar({ open, threads, activeId, onClose, onSelect, onNewChat, onRename, onArchive, onDelete, onSettings }: {
  ...
  function beginFolderRename(folder: ConversationFolder) {
  ...
  function commitFolderRename() {
    ...
    try {
      ...
      if (cleaned) renameFolder(editingFolderId, cleaned);
    } catch (cause) {
  ...
  function submitFolderCreate() {
    try {
      ...
      if (!cleaned) return;
      createFolder(cleaned, creatingUnder ?? null);
      setNewFolderName('');
      setCreatingUnder(undefined);
      setFolderError(null);
    } catch (cause) {
        ...
        <details className="thread-menu">
          ...
          <div className="thread-menu__panel">
            ...
            <label className="sidebar-menu-field">
              <span>Move to</span>
              <select aria-label={`Move ${thread.title} to folder`} value={folderState.assignments[thread.id] ?? ''} onChange={(event) => assignThread(thread.id, event.target.value || null)}>
                <option value="">Unfiled</option>
                {folderState.folders.map((folder) => <option key={folder.id} value={folder.id}>{folderPath(folder.id)}</option>)}
              </select>
            </label>
  ...
  function renderFolder(folder: ConversationFolder): JSX.Element {
    const children = allFolders.get(folder.id) ?? [];
    return <details className="folder-node" key={folder.id} open={!query.trim()}>
        ...
        {editingFolderId === folder.id ? (
          <input
            ...
            onKeyDown={(event) => {
              ...
              if (event.key === 'Escape') { setEditingFolderId(null); setEditingFolderName(''); }
            }}
      ...
      <div className="folder-node__children">
        ...
        {children.length === 0 && threadsInFolder(folder.id).length === 0 && <span className="folder-empty">Empty</span>}
      </div>
    </details>;
  }
    ...
    <>
      ...
      <aside className={`sidebar${open ? ' is-open' : ''}`} aria-label="Chat threads" aria-hidden={!open}>
        ...
        <div className="folder-toolbar">
          ...
          <button type="button" className="folder-toolbar__add" aria-label="Create folder" onClick={() => { setCreatingUnder(null); setFolderError(null); }}><Icon name="plus" size={15} /> New</button>
        </div>

        {creatingUnder !== undefined && (
          <form className="folder-create" onSubmit={(event) => { event.preventDefault(); submitFolderCreate(); }}>
            <input value={newFolderName} onChange={(event) => setNewFolderName(event.target.value)} autoFocus placeholder={creatingUnder ? 'Subfolder or nested/path' : 'Folder or Project/Section'} aria-label="New folder name" />
            <button type="submit">Create</button>
            <button type="button" onClick={() => { setCreatingUnder(undefined); setNewFolderName(''); }}>Cancel</button>
          </form>
        )}
