...
OAuth exchange, personal-token activation and disconnect are identity-changing writes. Before egress, the browser records a non-secret pending marker owned by the exact Worker pairing and clears only that pairing's cached ClickUp identity, so other same-origin tabs immediately stop advertising the old identity. The Worker independently tracks a monotonic connection epoch and settled epoch; authenticated `GET /clickup/oauth/connection-state` reports whether an identity-changing operation is still in flight without changing the legacy strict status payload.

If a write response is lost or times out, subsequent status reads and execution-grant acquisition first consult that Worker operation state. While the Worker reports `pending`, they fail closed with `connection_pending` and Settings remains explicitly **connection state unknown**; they never accept a possibly pre-commit status snapshot. Once the Worker reports the epoch settled, the browser clears its pending marker and performs a fresh authoritative status read. Older Workers that do not expose operation-state reporting use a conservative pairing-owned fallback barrier longer than the maximum reviewed provider-exchange path (two Worker request deadlines plus margin).

Cached ClickUp connection metadata is also pairing-owned. A late failed/read response from a superseded Worker pairing can neither overwrite nor erase a newer pairing's cached status. Legacy unowned cache entries are ignored until the current pairing refreshes them.
