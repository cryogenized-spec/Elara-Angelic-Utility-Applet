...
export function ChatAppearanceSettings({ value, onChange }: { value: ChatAppearancePreferences; onChange: (value: ChatAppearancePreferences) => void }) {
  ...
  return <div className="chat-appearance-settings">
    ...
    <div className="setting-card appearance-card">
      ...
      <label className="colour-field"><span>Activity accent</span><div><input type="color" aria-label="Generation activity accent colour" value={normaliseHex(value.generationActivityAccent, '#6EA8FF')} onChange={(event) => patch({ generationActivityAccent: event.target.value.toUpperCase() })}/><input aria-label="Generation activity accent hex" value={value.generationActivityAccent} maxLength={7} onChange={(event) => patch({ generationActivityAccent: event.target.value })} onBlur={() => patch({ generationActivityAccent: normaliseHex(value.generationActivityAccent, '#6EA8FF') })}/></div></label>
    </div>
    <div className="setting-card appearance-card">
