...
id: SYS-MEM
status: active
verified_commit: 2069436cff007c96e9c06021626fbe1245cbef2c
...
Automatic recall, deliberate `memory.recall`, and management `memory.lookup` share one conversation-to-memory scope resolver and one ranking engine. Query-bearing retrieval is relevance-gated: generic importance, CORE kind, pinning, recency or reinforcement cannot by themselves make a zero-overlap memory enter a topical recall result. Broad deliberate recall requests whose normalized query contains no substantive search terms remain bounded/unfiltered so questions such as “what do you remember about me?” still work. Recall is bound to the conversation captured when the turn is elected; current UI navigation is only a compatibility fallback for legacy callers without turn provenance. Automatic and deliberate recall update canonical recall telemetry only for selected memories; management lookup does not.
...
**Companion continuity Pass 2:** the master switch gates both conversational recall and organic formation; `explicit-only` disables organic formation while preserving explicit confirmed memory work, and `direct-only` disables automatic prompt injection while preserving deliberate `memory.recall`. The new read-only `memory.recall` capability gives Elara a deliberate way to search the same scoped durable-memory universe when the user asks what she remembers or when missing past context would materially improve the answer. Memory-policy reads hold a shared cross-tab Web Lock through recall projection/telemetry and the final organic commit; preference saves take the exclusive side of that lock. Callers also re-read the canonical preference before exposing recalled context or committing organic memory so unsupported-lock fallbacks fail closed rather than trusting a stale first read.
...
**Companion continuity Pass 4:** merged and production-certified.  query-bearing recall now requires substantive lexical overlap after conversational stopword removal, so unrelated “important” memories cannot enter a turn solely through CORE/pinned/importance/recency weight. `natural` automatic recall uses only that relevant lane. `proactive` uses the same relevant lane first and may then add at most one unrelated continuity anchor if budget remains. A continuity anchor must be an active, non-conflicted, established memory (never MICRO); it may be pinned, CORE, or high-confidence/high-importance CONTEXTUAL. Memories tagged with a sensitive category are never eligible for the unrelated proactive-anchor lane, although they may still be recalled when actually relevant to the user’s present query. Relevant memories always have first claim on item/character budgets, so the proactive anchor cannot evict topical context. A mid-flight recall-style change invalidates the automatic projection rather than exposing context selected under an older broader style.

**Companion continuity Pass 5 (current human-control change):** Settings now exposes the existing `memory-behavior` authority as a companion-facing **Memory & continuity** surface before the technical Memory Bank. The master switch controls conversational memory without deleting stored records or resetting subordinate choices. Remembering temperament exposes `Only when I ask / Selective / Natural / Attentive`; recall temperament exposes `Only when I ask / Naturally / Make connections`. Everyday and sensitive automatic-memory categories are individually switchable; sensitive categories remain visibly separated and default-off. Explicit confirmed `memory.save` remains a separate authority boundary, and credential/account material remains ineligible regardless of UI choices. The controls save automatically through the existing preference authority. Each UI action is applied as an atomic patch against the latest canonical `memory-behavior` record under the existing cross-tab policy lock/IndexedDB transaction, so a stale tab cannot overwrite unrelated newer choices. No second UI store or memory authority is introduced.
...
Retrieval excludes archived, expired and superseded records plus dormant `MICRO_OBSERVATION` evidence. Dormant established memories remain eligible at reduced weight when otherwise valid. Retrieval obeys folder ancestry plus explicit global-context policy. Query-bearing retrieval defaults to a relevance-gated mode; query-less ranking remains unfiltered for explicit authorities such as the separately user-consented Autonomy Context. Automatic `natural` recall is relevance-only. Automatic `proactive` recall may append one non-sensitive established continuity anchor after relevant results, never before them. Deliberate broad recall with no substantive query terms remains bounded/unfiltered. Default budget is eight records / 6,000 prose characters; hard caps are 20 / 20,000.
...
**Review hardening.** Identity collisions are checked transactionally on updates as well as creates, including singleton You kinds. UI aliases are normalized and individually length-validated. Credential-shaped and disabled-category source material (including explicit category tags, not just wording hints) is excluded before synthesis and projection through a shared policy predicate. A derived summary is withheld unless every referenced source exists, is retrievable in the current scope, and passes that predicate: a permitted subset cannot authorize blended claims from a private, deleted or blocked source. Rebuilds send canonical evidence only, never previous summary prose, and discard extraction output if the caller aborts before validation. Maintenance proposal evidence comes from canonical text, never a previous synthesized summary; a topic with no eligible evidence is reported unavailable and retained unchanged. Source excerpts include their framing within the 700-character cap, including unusually long titles.

**Local retrieval cost.** The automatic dossier lane skips all table reads for disabled/direct-only or query-less recall, and skips the extra canonical read for an empty cabinet. Otherwise it reads the local canonical and derived tables in addition to canonical recall. Evidence freshness checks sort canonical candidates for at most three selected files. Prompt and model-input budgets are bounded, but local scan cost grows with the latent corpus; no whole-corpus prompt is transmitted. A shared read snapshot or indexed evidence lookup is a future performance optimization, not a second authority.
