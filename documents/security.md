...
- Every signed ClickUp connection write is additionally ordered inside the Durable Object by its already-HMAC-covered `X-Elara-Timestamp`. The vault persists the highest admitted timestamp and rejects any older or equal start/exchange/personal-token/disconnect request before provider or account mutation. This prevents network delivery order from reversing browser intent; same-millisecond collisions fail closed rather than selecting an unsafe winner.
