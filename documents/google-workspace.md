...
- Drive provider fields entering a tool result are bounded (name, MIME type, description, timestamps, link, ETag text, parent count), so one hostile response cannot inflate the model-visible payload.
