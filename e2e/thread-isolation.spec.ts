...
test('starting a new thread isolates it from a still-running previous response', async ({ page }) => {
  ...
  await expect(page.getByRole('region', { name: 'Conversation' }).locator('.message')).toHaveCount(0);
  await new Promise((resolve) => setTimeout(resolve, 1800));
  await expect(page.getByRole('region', { name: 'Conversation' }).locator('.message')).toHaveCount(0);
  await expect(page.getByRole('region', { name: 'Conversation' })).not.toContainText('This belongs only to the old thread.');
  await expect(page.getByRole('region', { name: 'Conversation' })).not.toContainText('Old thread response.');
});
