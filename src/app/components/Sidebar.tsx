...
export function Sidebar({ open, threads, activeId, onClose, onSelect, onNewChat, onRename, onArchive, onDelete, onSettings }: {
  ...
  async function commitFolderRename() { if (!editingFolderId) return; try { const cleaned = editingFolderName.trim(); if (cleaned) await renameFolder(editingFolderId, cleaned); setEditingFolderId(null); setEditingFolderName(''); } catch (cause) { setFolderError(cause instanceof Error ? cause.message : 'Could not rename that folder.'); } }
  async function submitFolderCreate() { try { const cleaned = newFolderName.trim(); if (!cleaned) return; await createFolder(cleaned, creatingUnder ?? null); setNewFolderName(''); setCreatingUnder(undefined); setFolderError(null); } catch (cause) { setFolderError(cause instanceof Error ? cause.message : 'Could not create that folder.'); } }
  async function changeMove(folderId: string, parentId: string | null) { try { await moveFolder(folderId, parentId); setFolderError(null); } catch (cause) { setFolderError(cause instanceof Error ? cause.message : 'Could not move that folder.'); } }
  async function changeScope(folderId: string, scope: ConversationFolder['contextScope']) { try { await setContextScope(folderId, scope); setFolderError(null); } catch (cause) { setFolderError(cause instanceof Error ? cause.message : 'Could not change memory scope.'); } }
  async function changeAssignment(threadId: string, folderId: string | null) { try { await assignThread(threadId, folderId); setFolderError(null); } catch (cause) { setFolderError(cause instanceof Error ? cause.message : 'Could not move that conversation.'); } }
  async function changeDelete(folderId: string) { try { await removeFolder(folderId); setFolderError(null); } catch (cause) { setFolderError(cause instanceof Error ? cause.message : 'Could not delete that folder.'); } }
  ...
  function renderFolder(folder: ConversationFolder): ReactElement {
    ...
    return <details className="folder-node" key={folder.id} open={!query.trim()}><summary className="folder-row"><span className="folder-row__chevron" aria-hidden="true">›</span><span className="folder-row__icon" aria-hidden="true">□</span>{editingFolderId === folder.id ? <input className="folder-row__input" value={editingFolderName} maxLength={80} autoFocus aria-label="Folder name" onChange={(event) => setEditingFolderName(event.target.value)} onClick={(event) => event.stopPropagation()} onKeyDown={(event) => { if (event.key === 'Enter') void commitFolderRename(); if (event.key === 'Escape') { setEditingFolderId(null); setEditingFolderName(''); } }} onBlur={() => void commitFolderRename()} /> : <span className="folder-row__name">{folder.name}</span>}<span className="folder-row__count">{threadsInFolder(folder.id).length}</span></summary><details className="folder-menu" onClick={(event) => event.stopPropagation()}><summary aria-label={`Folder actions for ${folderPath(folder.id)}`}><Icon name="dots" size={16} /></summary><div className="folder-menu__panel"><button type="button" onClick={() => { setCreatingUnder(folder.id); setFolderError(null); }}>New subfolder</button><button type="button" onClick={() => beginFolderRename(folder)}>Rename</button><label className="sidebar-menu-field"><span>Move to</span><select aria-label={`Move folder ${folderPath(folder.id)}`} value={folder.parentId ?? ''} onChange={(event) => void changeMove(folder.id, event.target.value || null)}><option value="">Root</option>{moveTargetOptions(folder.id).map((target) => <option key={target.id} value={target.id}>{folderPath(target.id)}</option>)}</select></label><label className="sidebar-menu-field"><span>Memory scope</span><select aria-label={`Memory scope for ${folderPath(folder.id)}`} value={folder.contextScope} onChange={(event) => void changeScope(folder.id, event.target.value as ConversationFolder['contextScope'])}><option value="folder">Folder only</option><option value="global">Global</option></select><small>Applies to durable-memory retrieval; normal chat history remains thread-scoped.</small></label><button type="button" className="is-danger" onClick={() => void changeDelete(folder.id)}>Delete folder</button></div></details><div className="folder-node__children">{children.map(renderFolder)}{threadsInFolder(folder.id).filter(threadMatches).map((thread) => renderThread(thread, true))}{children.length === 0 && threadsInFolder(folder.id).length === 0 && <span className="folder-empty">Empty</span>}</div></details>;
  }
