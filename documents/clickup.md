...
Cached ClickUp connection metadata is also pairing-owned and revision-monotonic. Failure cleanup compare-and-clears only the exact cache snapshot a request started with; a late failed/read response from a superseded Worker or an older same-pairing request can neither overwrite nor erase a newer refresh. Legacy unowned cache entries are ignored until the current pairing refreshes them.
