...
export function ChatAppearanceSettings({ value, onChange }: { value: ChatAppearancePreferences; onChange: (value: ChatAppearancePreferences) => void }) {
  ...
  function patch(patch: Partial<ChatAppearancePreferences>) { onChange({ ...value, ...patch }); }
  ...
  return <div className="chat-appearance-settings">
    <div className="setting-card appearance-card">
      ...
      {value.chatBackgroundMode === 'solid' && <label className="colour-field"><span>Background colour</span><div><input type="color" value={normaliseHex(value.chatBackgroundValue, '#050507')} onChange={(event) => patch({ chatBackgroundValue: event.target.value.toUpperCase() })}/><input value={value.chatBackgroundValue} maxLength={7} aria-label="Chat background hex" onChange={(event) => patch({ chatBackgroundValue: event.target.value })} onBlur={() => patch({ chatBackgroundValue: normaliseHex(value.chatBackgroundValue, '#050507') })}/></div></label>}
      ...
      <label className="setting-range"><span>Background opacity</span><input type="range" min={0} max={1} step={0.01} value={value.chatBackgroundOpacity} onChange={(event) => patch({ chatBackgroundOpacity: Number(event.target.value) })}/><output>{Math.round(value.chatBackgroundOpacity * 100)}%</output></label>
      <label className="setting-range"><span>Readability overlay</span><input type="range" min={0} max={0.9} step={0.01} value={value.chatBackgroundOverlay} onChange={(event) => patch({ chatBackgroundOverlay: Number(event.target.value) })}/><output>{Math.round(value.chatBackgroundOverlay * 100)}%</output></label>
      <label className="setting-range"><span>Background blur</span><input type="range" min={0} max={24} step={1} value={value.chatBackgroundBlur} onChange={(event) => patch({ chatBackgroundBlur: Number(event.target.value) })}/><output>{value.chatBackgroundBlur}px</output></label>
    </div>
    <div className="setting-card appearance-card">
      <strong>Elara messages</strong><span>Independent assistant text colour.</span>
      <label className="colour-field"><span>Text colour</span><div><input type="color" value={normaliseHex(value.assistantTextColor, '#F7F8FF')} onChange={(event) => patch({ assistantTextColor: event.target.value.toUpperCase() })}/><input value={value.assistantTextColor} maxLength={7} onChange={(event) => patch({ assistantTextColor: event.target.value })} onBlur={() => patch({ assistantTextColor: normaliseHex(value.assistantTextColor, '#F7F8FF') })}/></div></label>
    ...
    <div className="setting-card appearance-card">
      ...
      <label className="colour-field"><span>Text colour</span><div><input type="color" value={normaliseHex(value.userTextColor, '#F7F8FF')} onChange={(event) => patch({ userTextColor: event.target.value.toUpperCase() })}/><input value={value.userTextColor} maxLength={7} onChange={(event) => patch({ userTextColor: event.target.value })} onBlur={() => patch({ userTextColor: normaliseHex(value.userTextColor, '#F7F8FF') })}/></div></label>
      <label className="colour-field"><span>Surface colour</span><div><input type="color" value={normaliseHex(value.userSurfaceColor, '#28344F')} onChange={(event) => patch({ userSurfaceColor: event.target.value.toUpperCase() })}/><input value={value.userSurfaceColor} maxLength={7} onChange={(event) => patch({ userSurfaceColor: event.target.value })} onBlur={() => patch({ userSurfaceColor: normaliseHex(value.userSurfaceColor, '#28344F') })}/></div></label>
      ...
      <label className="setting-range"><span>Surface opacity</span><input type="range" min={0.2} max={1} step={0.01} value={value.userSurfaceOpacity} onChange={(event) => patch({ userSurfaceOpacity: Number(event.target.value) })}/><output>{Math.round(value.userSurfaceOpacity * 100)}%</output></label>
    </div>
  </div>;
}
