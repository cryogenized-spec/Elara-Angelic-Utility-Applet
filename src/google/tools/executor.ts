...
import type { GoogleOAuthAuthority, GoogleOAuthStatus } from '../oauth/contracts';
...
function authorizationNeeded(status: GoogleOAuthStatus, capability: string): boolean {
  return !status.grantedCapabilities.includes(safeCapability(capability) as never) || status.state === 'disconnected' || status.state === 'needs-consent' || status.state === 'revoked' || status.state === 'reauthorization-required';
}
...
export async function executeGoogleTool(call: GoogleToolCall, options: GoogleToolExecutorOptions): Promise<GoogleToolExecutionResult> {
  ...
  if (!parsed.success) {
    return { ok: false, correlationId: id, code: 'INVALID_TOOL_CALL', failure: classifyGoogleToolFailure({ kind: 'validation' }) };
  }
  ...
  if (authorizationNeeded(status, descriptor.capability)) {
  ...
  if (decision.requiresConfirmation) {
    ...
    if (!options.confirm) {
      return { ok: false, correlationId: id, tool: validCall.tool, code: 'CONFIRMATION_REQUIRED', failure: classifyGoogleToolFailure({ kind: 'confirmation' }), confirmation };
    }
    let approved = false;
    try {
    ...
    if (!approved) {
      return { ok: false, correlationId: id, tool: validCall.tool, code: 'USER_DECLINED', failure: classifyGoogleToolFailure({ kind: 'confirmation' }), confirmation };
    }
  }

  const handler = options.handlers[descriptor.name];
  if (!handler) {
    return { ok: false, correlationId: id, tool: validCall.tool, code: 'HANDLER_UNAVAILABLE', failure: classifyGoogleToolFailure({ kind: 'unknown' }) };
  }

  try {
    const result = await handler({ tool: descriptor.name, descriptor, capability: descriptor.capability, risk: descriptor.risk, arguments: args });
