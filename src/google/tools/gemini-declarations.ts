...
const toolProperties: Record<string, Record<string, unknown>> = {
  ...
  'sheets.createSpreadsheet': { title: stringProperty('New spreadsheet title.'), firstSheetTitle: stringProperty('Optional title for the initial sheet/tab.') },
  'sheets.addSheet': { spreadsheetId: stringProperty('Spreadsheet id.'), title: stringProperty('New sheet/tab title.'), rowCount: { type: 'integer', minimum: 1, maximum: 100000 }, columnCount: { type: 'integer', minimum: 1, maximum: 1000 } },
  'sheets.writeRange': { spreadsheetId: stringProperty('Spreadsheet id.'), range: stringProperty('A1 range to write.'), values: arrayProperty('Rows of bounded primitive cell values (string, finite number, boolean, or null).', { type: 'array' }), inputMode: { type: 'string', enum: ['literal', 'userEntered'], description: 'Optional input interpretation. Defaults to literal (RAW). Use userEntered only when Sheets should intentionally parse formulas/dates/numbers like the UI.' } },
  'sheets.appendRows': { spreadsheetId: stringProperty('Spreadsheet id.'), range: stringProperty('A1 range used for append.'), values: arrayProperty('Rows of bounded primitive cell values (string, finite number, boolean, or null).', { type: 'array' }), inputMode: { type: 'string', enum: ['literal', 'userEntered'], description: 'Optional input interpretation. Defaults to literal (RAW).' } },
  'sheets.updateCell': { spreadsheetId: stringProperty('Spreadsheet id.'), range: stringProperty('A1 cell to write.'), value: { type: 'string', maxLength: 50_000, description: 'Bounded cell text. Defaults to a literal RAW write, so a leading = is text unless inputMode=userEntered is explicitly requested.' }, inputMode: { type: 'string', enum: ['literal', 'userEntered'], description: 'Defaults to literal (RAW). userEntered intentionally enables Sheets parsing, including formulas.' } },
...
const requiredByTool: Record<string, readonly string[]> = {
  ...
  'docs.getDocument': ['documentId'], 'docs.inspectDocument': ['documentId'], 'docs.createDocument': ['title'], 'docs.insertText': ['documentId', 'tabId', 'revisionId', 'index', 'text'], 'docs.appendParagraph': ['documentId', 'tabId', 'revisionId', 'text'], 'docs.replaceText': ['documentId', 'tabId', 'revisionId', 'findText', 'replaceText'], 'docs.batchUpdate': ['documentId', 'requests'],
  ...
  'sheets.getSpreadsheet': ['spreadsheetId'], 'sheets.readRange': ['spreadsheetId', 'range'], 'sheets.createSpreadsheet': ['title'], 'sheets.addSheet': ['spreadsheetId', 'title'], 'sheets.writeRange': ['spreadsheetId', 'range', 'values'], 'sheets.appendRows': ['spreadsheetId', 'range', 'values'], 'sheets.updateCell': ['spreadsheetId', 'range', 'value'], 'sheets.insertRows': ['spreadsheetId', 'sheetId', 'startIndex', 'count'], 'sheets.batchUpdate': ['spreadsheetId', 'requests'],
