...
describe('ClickUp signed webhook cache invalidation', () => {
  ...
  it('does not resurrect a task deleted while the first cold index page is in flight', async () => {
    ...
    await connectThroughPublicWorker();
    const pendingSearch = internalSearch();

    for (let attempt = 0; attempt < 100 && !firstPageStarted; attempt += 1) {
      await new Promise<void>((resolve) => setTimeout(resolve, 2));
    }
    expect(firstPageStarted).toBe(true);
    ...
    const payload = JSON.stringify({
      ...
      task_id: 'task-repair',
      webhook_id: WEBHOOK_ID,
      history_items: [{ id: 'history-cold-delete-race' }],
    });
    ...
    expect(JSON.stringify(await result.clone().json())).not.toContain('STALE_COLD_PAGE');
