...
OAuth exchange, personal-token activation and disconnect are identity-changing writes. If one has an ambiguous network/protocol failure after request egress, the browser records a non-secret pending marker owned by the exact Worker pairing, clears only that pairing's cached ClickUp identity, and enters an explicit **connection state unknown** mode. The settle barrier lasts longer than the maximum remaining reviewed provider-exchange path (currently two Worker request deadlines plus margin). During that barrier, authoritative status reads, execution-grant acquisition and additional connection changes fail closed with `connection_pending`; they do not accept a possibly pre-commit status snapshot. After the barrier expires, Refresh status performs a fresh authoritative read.
