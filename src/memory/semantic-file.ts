...
export async function writeSemanticFile(file: SemanticMemoryFile, expectedVersion: number): Promise<{ file: SemanticMemoryFile; changed: boolean }> {
  return db.transaction('rw', db.semanticMemories, async () => {
    ...
    if (expectedVersion === 0) {
      if (current) throw new Error('Semantic memory file already exists.');
      if ((await table().count()) >= SEMANTIC_MAX_FILES) throw new Error('Semantic memory cabinet capacity reached.');
      // Identity-collision guard, rechecked inside the transaction: a
      // concurrent rebuild that resolved to "create" from a stale read must
      // fail closed instead of filing the same concept twice.
      for (const existing of await table().toArray()) {
        const parsed = semanticMemoryFileSchema.safeParse(existing);
        if (parsed.success && parsed.data.kind === file.kind && semanticIdentityOverlaps(parsed.data, file)) {
