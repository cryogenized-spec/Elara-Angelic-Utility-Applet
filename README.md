...
These are browser-visible public identifiers, not OAuth client secrets. The CI workflow injects the variables into the Vite build. A production `main` build now fails closed when `VITE_GOOGLE_CLIENT_ID` is missing, so GitHub Pages cannot silently publish a Workspace UI whose account button can never open Google authorization. The Google Web client must authorize the Pages origin `https://cryogenized-spec.github.io`; Picker credentials should be restricted to the same web origin and the Google Picker API.
