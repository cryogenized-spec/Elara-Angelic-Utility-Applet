    ...
    try {
      while (true) {
        ...
        while (boundary >= 0) {
          ...
          if (eventType === 'tool-call') { const callId = readString(event, 'call_id'); const name = readString(event, 'name'); const args = asRecord(event.arguments); if (callId && name && interactionId) yield { type: 'tool-call', interactionId, index: stepIndex(event), callId, name, arguments: args }; sawRequiresAction = true; continue; }
