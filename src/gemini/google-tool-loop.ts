...
function registryRisk(tool: GoogleToolName): GoogleToolRisk {
  const descriptor = googleToolRegistry.find((entry) => entry.name === tool);
  if (!descriptor) throw new Error('Tool is not registered.');
  return descriptor.risk;
}

function normalizeTools(tools: readonly GoogleToolName[] | undefined, readOnly: boolean): readonly GoogleToolName[] {
  const requested = tools?.length ? [...tools] : [...GOOGLE_READ_TOOL_NAMES];
  const unique = [...new Set(requested)];
  for (const tool of unique) {
    if (readOnly && registryRisk(tool) !== 'read') throw new Error(`Tool ${tool} is not permitted in read-only mode.`);
  }
  return unique;
}
...
function executorOptions(options: GoogleToolLoopOptions): GoogleToolExecutorOptions {
  return {
    ...
    handlers: options.executor?.handlers ?? googleReadToolHandlers,
...
export async function* streamGoogleToolLoop(
  request: GeminiTurnRequest,
  options: GoogleToolLoopOptions = {},
  signal?: AbortSignal,
  ...
  while (true) {
    ...
    for await (const event of stream) {
      ...
      const result = await executeGoogleTool(
        { tool: event.name as GoogleToolName, arguments: event.arguments },
        executeOptions,
      );
      ...
      if (result.ok) {
        continuation = {
          model: request.model,
          previousInteractionId: event.interactionId,
          result: { callId: event.callId, name: event.name, result: result.result },
          systemInstruction: request.systemInstruction,
          generationConfig: request.generationConfig,
          tools,
        };
        ...
        continuation = {
          ...
          result: {
            ...
            result: {
              ...
              requiresUserAction: result.failure.requiresUserAction,
            },
          },
          systemInstruction: request.systemInstruction,
          generationConfig: request.generationConfig,
