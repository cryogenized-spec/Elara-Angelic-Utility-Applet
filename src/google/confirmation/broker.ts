...
export function requestGoogleToolConfirmation(request: WriteConfirmationRequest, signal?: AbortSignal): Promise<boolean> {
  ...
  return new Promise((resolve) => {
    ...
    host.setAttribute('aria-label', 'Google action confirmation');

    const riskLabel = request.risk === 'send' ? 'Send' : request.risk === 'destructive' ? 'Destructive change' : 'Change';
    host.innerHTML = `
      <div class="roleplay-confirmation__heading"><span>✦</span><strong>Elara proposes a ${riskLabel.toLowerCase()}</strong></div>
      ...
      <div class="roleplay-confirmation__actions">
        ...
        <button type="button" data-decision="accept" class="roleplay-confirmation__accept">✓ Approve</button>
      </div>`;
