...
Authorization remains capability-driven even though Settings onboarding is bundled. The Google Settings surface uses one explicit user gesture to request the current Workspace capability set together so Google's granular consent screen can present the permissions in one place. Google remains authoritative for which individual scopes are actually granted; Elara records the requested bundle as local user intent and computes effective capabilities only from the intersection with the provider-returned scope set. Contextual capability authorization still exists for later features or missing grants. A paired Worker may report a broader provider scope set from another device, but provider state alone never inserts a new local capability without an explicit Elara authorization action.
...
The browser authorization record remains version 3 under the historical `elara.google.authorization.v2` key. It stores only enabled capabilities, current provider scopes, optional account display metadata, recovery state and timestamps. Browser access tokens remain module-memory-only and are never persisted. Each live browser access-token session is bound to the normalized Google account identity verified for that token. If same-origin authorization metadata changes to another account in a different tab, the old tab invalidates its in-memory token before reporting `sessionReady` or performing another provider request. Silent existing-grant refresh requires the persisted account identity to be present, verifies the returned token's account against it, then reloads the shared authorization record immediately before committing the refreshed session. If another same-origin tab changed accounts while GIS/userinfo was pending, the stale refresh aborts without overwriting or marking the newer account for reauthorization. `GoogleOAuthStatus.sessionReady` is derived from this live, identity-bound in-memory access-token session and is never persisted as consent evidence.
...
`e2e/google-oauth-settings.spec.ts` exercises the real Settings flow by stubbing only Google's external GIS/userinfo boundary. It pins one-click bundled Workspace consent, one GIS token request per user gesture, live-session gating, reload-time session refresh, provider-scope persistence, and disconnect cleanup. `src/google/oauth/authority.test.ts` separately pins both direct identity-only `google.account` authorization and bundled Workspace authorization, including partial-grant handling. Durable Worker behavior is covered by unit + workerd tests because production credentials must never be required by CI.
