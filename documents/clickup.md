...
If a write response is lost or times out, subsequent status reads and execution-grant acquisition consult that Worker operation state. While the Worker reports `pending`, they fail closed with `connection_pending` and Settings remains explicitly **connection state unknown**. A status snapshot from a new Worker is accepted only when the connection epoch is settled **and unchanged before and after the status read**. That closes the cross-tab race where another tab could begin a replacement after the first status check but before the old account snapshot returned. Once the same epoch remains settled, the browser clears only the pending marker it owns and persists the fresh authoritative status. Older Workers that do not expose operation-state reporting use a conservative pairing-owned fallback barrier longer than the maximum reviewed provider-exchange path (two Worker request deadlines plus margin).
...
Public OAuth bodies are streamed under a byte ceiling before signature verification/forwarding. Signed writes use timestamp + nonce + body and have durable replay protection. The already-HMAC-covered write timestamp is also the Worker-side ordering generation for ClickUp connection authority. The browser captures that timestamp at the start of the connection gesture, before async Worker/token preflight, and signs that original intent-time value even if network egress is delayed. The vault durably stores the newest admitted timestamp and rejects an older or equal connection write with `connection_superseded` before it can create OAuth state, call ClickUp, disconnect, or replace the credential. This specifically closes delayed-network reordering across same-origin tabs without adding a new request-body field, so older browser builds remain wire-compatible. Equal-millisecond competing writes fail closed and require retry rather than choosing an arrival-order winner. OAuth state is random, redirect-bound, short-lived and single-use. A new Connect gesture replaces older pending states and advances the connection epoch, so an older popup or already-in-flight exchange cannot later overwrite the newer authorization.
