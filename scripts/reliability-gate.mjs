...
const organicMemorySafetySource = readFileSync(join(root, 'src/memory/organic-observer.ts'), 'utf8');
if (!organicMemorySafetySource.includes('containsLuhnValidCardNumber') || !organicMemorySafetySource.includes('AKIA|ASIA') || !organicMemorySafetySource.includes('eyJ[A-Za-z0-9_-]')) throw new Error('Reliability gate: automatic memory must retain deterministic bare-secret and card-number rejection.');
