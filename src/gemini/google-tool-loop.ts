...
function executorOptions(options: GoogleToolLoopOptions): GoogleToolExecutorOptions {
  return { oauth: options.executor?.oauth ?? googleOAuthAuthority, handlers: options.executor?.handlers ?? { ...googleReadToolHandlers, ...roleplayWorldToolHandlers }, confirm: options.executor?.confirm ?? requestRoleplayConfirmation, now: options.executor?.now };
}
...
export async function* streamGoogleToolLoop(request: GeminiTurnRequest, options: GoogleToolLoopOptions = {}, signal?: AbortSignal): AsyncGenerator<GeminiStreamEvent> {
  ...
  while (true) {
    ...
    for await (const event of stream) {
      ...
      if (result.ok) {
        continuation = { model: request.model, previousInteractionId: event.interactionId, result: { callId: event.callId, name: event.name, result: result.result }, systemInstruction, generationConfig: request.generationConfig, tools };
      } else {
        continuation = { model: request.model, previousInteractionId: event.interactionId, result: { callId: event.callId, name: event.name, result: { ok: false, code: result.code, message: result.failure.message, requiresUserAction: result.failure.requiresUserAction } }, systemInstruction, generationConfig: request.generationConfig, tools };
      }
      break;
    }
