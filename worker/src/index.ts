...
function toSafeEvent(raw: unknown): { name: string; data: SafeEvent } | null { const event = asRecord(raw); const eventType = stringValue(event, 'event_type') ?? stringValue(event, 'type') ?? ''; const id = interactionId(event); if (eventType === 'interaction.created') { const interaction = asRecord(event.interaction); return { name: 'interaction.created', data: { event_type: eventType, interaction: { id: stringValue(interaction, 'id') ?? id, status: stringValue(interaction, 'status'), model: stringValue(interaction, 'model') } } }; } if (['interaction.in_progress','interaction.status_update','interaction.status','interaction.updated','interaction.requires_action'].includes(eventType)) return { name: eventType, data: { event_type: eventType, interaction_id: id, status: stringValue(event, 'status') ?? stringValue(asRecord(event.interaction), 'status') } }; if (eventType === 'step.start') { const step = asRecord(event.step); const summary = Array.isArray(step.summary) ? step.summary.map((item) => ({ text: stringValue(asRecord(item), 'text') })).filter((item) => item.text) : []; return { name: 'step.start', data: { event_type: eventType, interaction_id: id, index: indexOf(event), step: { index: numberValue(step, 'index') ?? indexOf(event), type: stringValue(step, 'type') ?? 'other', summary, signature: stringValue(step, 'signature') } } }; } if (eventType === 'step.delta') { const delta = asRecord(event.delta); const deltaType = stringValue(delta, 'type'); if (deltaType === 'text' || deltaType === 'thought_summary' || deltaType === 'thought_signature') return { name: 'step.delta', data: { event_type: eventType, interaction_id: id, index: indexOf(event), delta: { type: deltaType, text: stringValue(delta, 'text'), signature: stringValue(delta, 'signature') } } }; return null; } if (eventType === 'step.stop') return { name: 'step.stop', data: { event_type: eventType, interaction_id: id, index: indexOf(event) } }; if (eventType === 'interaction.completed') { const interaction = asRecord(event.interaction); const usage = asRecord(interaction.usage); return { name: 'interaction.completed', data: { event_type: eventType, interaction: { id: stringValue(interaction, 'id') ?? id, status: stringValue(interaction, 'status') ?? 'completed', usage: Object.keys(usage).length > 0 ? usage : undefined } } }; } if (eventType === 'error') { const error = asRecord(event.error); return { name: 'error', data: { event_type: eventType, error: { message: stringValue(error, 'message') ?? 'Gemini returned a streaming error.' } } }; } return null; }
function toGenerationConfig(config: z.infer<typeof requestSchema>['generationConfig']) { if (!config) return undefined; const generationConfig: Record<string, unknown> = {}; if (config.thinkingLevel) generationConfig.thinking_level = config.thinkingLevel; if (config.thinkingSummaries) generationConfig.thinking_summaries = config.thinkingSummaries; if (config.maxOutputTokens !== undefined) generationConfig.max_output_tokens = config.maxOutputTokens; if (config.seed !== undefined) generationConfig.seed = config.seed; if (config.stopSequences?.length) generationConfig.stop_sequences = config.stopSequences; return Object.keys(generationConfig).length > 0 ? generationConfig : undefined; }
...
async function handleGemini(request: Request, env: Env): Promise<Response> {
  ...
  const origin = request.headers.get('Origin'); if (origin && !allowedOrigin(request, env)) return jsonResponse(request, env, { code: 'authz', message: 'Origin is not authorized.' }, 403);
  const contentType = request.headers.get('Content-Type') ?? ''; if (!contentType.toLowerCase().includes('application/json')) return jsonResponse(request, env, { code: 'validation', message: 'Content-Type must be application/json.' }, 415);
  const payload = await request.json().catch(() => null); const parsed = requestSchema.safeParse(payload); if (!parsed.success) return jsonResponse(request, env, { code: 'validation', message: 'Request did not satisfy the approved Gemini contract.' }, 400);
  ...
  const stream = await client.interactions.create({ model: parsed.data.model, input: parsed.data.input, system_instruction: systemInstruction, previous_interaction_id: parsed.data.previousInteractionId, generation_config: toGenerationConfig(parsed.data.generationConfig), stream: true, store: true });
