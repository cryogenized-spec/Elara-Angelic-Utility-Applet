...
- Every ClickUp identity-changing write marks the exact Worker pairing pending before egress with a unique browser operation ID and clears only that pairing's cached identity. Pending-marker cleanup is compare-and-clear by operation ID, so an older tab cannot clear a newer tab's in-flight replacement. The Worker tracks durable connection/settled epochs; after an ambiguous response, status reads and execution grants remain fail-closed until authenticated operation-state reporting confirms settlement. Older Workers use the bounded timer only as a compatibility fallback.
- Every signed ClickUp connection write is additionally ordered inside the Durable Object by its already-HMAC-covered `X-Elara-Timestamp`. The browser captures this timestamp before any async connection preflight so it represents gesture order rather than eventual network-send time. The vault persists the highest admitted timestamp and rejects any older or equal start/exchange/personal-token/disconnect request before provider or account mutation. This prevents network delivery order from reversing browser intent; same-millisecond collisions fail closed rather than selecting an unsafe winner.
