...
Grouped mutation confirmation is fail-closed for human review. Multi-action batches start with every item unselected and expose no `Approve all` shortcut; the user selects intended mutations individually. Tainted single actions also start unselected. Confirmation retains the full validated mutation detail but renders it as human-readable provider/action labels and field/value review text rather than raw JSON, dotted tool identifiers or schema/debug payloads. Purpose-built content reviews (for example email bodies, ClickUp comments, memory prose and single-cell text) remain verbatim so the human approves the actual content. Short confirmations stay compact; substantial reviews switch to a viewport-aware sheet with the heading and decision controls outside the scrolling review surface, including safe-area-aware mobile layout.
...
The human [Kanban workspace](./kanban/README.md) is a separate UI/projection over the same semantic Tasks service, not another provider mutation authority. Its small model-visible awareness surface is described in §11 and deliberately reuses the canonical registry. Its reviewed `src/kanban/google-port.ts` boundary requires effective capabilities, an identified account and a live session before access, and uses only the OAuth authority's noninteractive existing-grant transport; background reconciliation never initiates consent. Human board mutations bind that service boundary to the displayed board account and recheck it before transport acquisition and provider fetch, so another same-origin tab cannot retarget a mutation by switching Google accounts after the UI action began. Account-keyed snapshots/rules in `src/kanban/store.ts` are a cache/local orchestration-metadata authority only. Elara may attach browser-local creation/first-seen timestamps, due time/time-zone hints and structured label identities to the task projection; Google remains authoritative for task title, notes, status, hierarchy and date-only `scheduledDate`. App-only metadata is merged back onto matching provider tasks after reconciliation and is never serialized as undeclared Google Task fields. Identity changes prune other-account cache rows and provider disconnect/revocation clears the Kanban cache, while a temporary locked session preserves the same identified account's hidden cache. Explicit Save actions admit human writes, and typed confirmation admits destructive human actions. Model writes retain the existing executor and grouped broker.

`tasks.updateTask` optionally accepts the current task ETag and propagates `If-Match` for conditional PATCH. Successful Tasks mutations emit board invalidation; failed/412 writes never emit success. An enabled local subroutine can surface earlier scheduled dates as overdue memo context without changing or moving the provider task. When that persisted provider-derived memo is present, the App marks the turn `untrustedExternalContext`; the existing Gemini tool-loop taint authority therefore treats the first model tool batch as already tainted, so memo text cannot silently authorize a mutation. Empty overdue memos are not injected into model context. See the supporting guide for refresh cadence, browser-local privacy and limitations.
...
### Model-visible Kanban awareness

Elara now has a browser-only Kanban awareness surface in the canonical model tool registry: `kanban.inspect`, `kanban.refresh`, `kanban.locate`, and `kanban.focus`. All four are read-risk tools gated by the existing `tasks.read` capability. They expose the current account's bounded board projection, reconciliation state and provider identities without creating another task authority.

Provider-derived board content is returned as `trust: untrusted-external`, so the ordinary tool-loop provenance/confirmation boundary applies if later model reasoning proposes a mutation. Every model-facing board read rechecks the current Google identity after loading the account-keyed projection so an account switch cannot race old-account data into the turn. `kanban.refresh` reuses the existing manual board reconciliation path and cannot initiate consent. `kanban.focus` is application presentation only: after validating the list/task exists in the synchronized projection and confirming the originating generation is still active, it opens the human Kanban and scrolls/highlights that DOM target. Google task/list changes continue exclusively through the reviewed `tasks.*` tools, schemas, OAuth admission and confirmation broker; there are no Kanban-branded provider mutation tools.
