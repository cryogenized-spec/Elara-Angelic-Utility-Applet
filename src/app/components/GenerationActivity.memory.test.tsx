...
describe('Generation Activity memory presentation', () => {
  it('uses the dedicated memory glyph semantic for deliberate tools and organic capture', () => {
    renderExpanded({
    ...
    expect(container.querySelectorAll('[data-activity-glyph="memory"]')).toHaveLength(2);
  });

  it('uses the same memory glyph semantic for recalled durable context without relabeling it as a tool', () => {
    renderExpanded({
    ...
    expect(container.querySelectorAll('[data-activity-glyph="memory"]')).toHaveLength(1);
  });
});
