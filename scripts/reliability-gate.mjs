...
if (!clickUpOauthVaultSource.includes("name: 'AES-GCM'") || !clickUpOauthVaultSource.includes('CLICKUP_OAUTH_VAULT_KEY') || !clickUpOauthVaultSource.includes('encryptProviderCredential') || !clickUpOauthVaultSource.includes('decryptProviderCredential') || !clickUpOauthVaultSource.includes('providerCredentialSchema')) throw new Error('Reliability gate: ClickUp provider credentials and their discriminated kind must remain AES-GCM encrypted with the dedicated vault key.');
